CREATE Procedure [dbo].[PaymentDataEntrySearchReceiptV3]
	@WireID int --READY
As   
  Set nocount on;
	...
	select W.WireId, W.AgSenderCode as AgencyCode, W.AgPayerCode as PayerCode, W.AgSenderSeq,
		...
		' ' as BeneficiaryId, --ISNULL(prt.RcvIdType + w.RcvIdNumber,'') as BeneficiaryId ,
		ISNULL(r.CPF,'') as CPF,
		w.AccountType 
	from Wires as W  
	inner join Senders as S on (S.LSenderId = W.LSenderId)
	inner join Receivers as R on (R.LReceiverId = W.LReceiverId)  
	--LEFT JOIN wiresearch.dbo.agpayerrcvidtypes prt on w.AgPayerRcvIdTypeRecordId = prt.agpayerRcvIdTypeRecordId
