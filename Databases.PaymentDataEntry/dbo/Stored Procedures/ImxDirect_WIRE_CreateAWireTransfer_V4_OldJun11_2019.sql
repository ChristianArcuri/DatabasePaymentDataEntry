CREATE procedure [dbo].[ImxDirect_WIRE_CreateAWireTransfer_V4_OldJun11_2019]
(
...
BEGIN TRY 
  ...
  SELECT @FXPointsAdded         = CASE WHEN PriceAffectedBy = 'FLEX_FX' then FlexPriceValueApplied else 0 end,
	     @FXChangeCost          = CASE WHEN PriceAffectedBy = 'FLEX_FX' then FlexPriceCost  else 0 end,
	     ...
	     @FeeChange             = CASE WHEN PlanFeeAmount <> WireFeeAmount then 1 else 0 end,
	     @CostToAgent           = CASE WHEN PriceAffectedBy like 'FLEX%' and PlanCommissionAmount <> WireCommissionAmount then 1 else 0 end,
	     @CostToCustomer        = CASE WHEN PriceAffectedBy like 'FLEX%' and PlanFeeAmount <> PlanFeeAmount  then 1 else 0 end,
  ...
  SET @SndFullName =rtrim(rtrim(@SndFirstName)+' '+rtrim(@SndLast1)+' '+Rtrim(@SndLast2))
