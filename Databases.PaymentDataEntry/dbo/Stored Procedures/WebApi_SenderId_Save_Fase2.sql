CREATE PROCEDURE [dbo].[WebApi_SenderId_Save_Fase2]
...
BEGIN
  ...
  -- only thing needed is IdTypeName - Luis Gamez
  ...
  IF @@ROWCOUNT = 0
	...
	EXEC SQLMain.WireTransac.dbo.Comp_SenderId_InsEdit_fase2   
											@NewId,   -- to check whether to update/add
											@CompSenderIdRecordId OUTPUT, -- will return ID of inserted record
											@SenderGroupId, -- get from line 76 -77
										    @IdType,
											@IdTypeName, -- get it ffrom line 49
											@IdNumber , 
										    @IdTypeDesc , 
										    @IdCountry , 
										    @IdState , 
