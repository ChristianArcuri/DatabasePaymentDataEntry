/**
    Name:           Bridge_GetFinalUpdateData
    Author:         Agustin Aguilar
    Written:        6/16/2023
    Purpose:        Gets a FinalUpdateData to update Wire, 
                    based on the legacy SP [WebApi_WireFinalUpdate_withPayerWireExpiration]

    Edit History:   6/16/2023 - Aguilar Agustin
                        + Initial creation.
					6/22/2023 - Aguilar Agustin
						+ Add output parameters to spi_Create_CumulativeSummaryAmountsToday_DestCurrency
**/


CREATE PROCEDURE [dbo].[Bridge_GetFinalUpdateData]
...
@WireReadyToChargeSender bit OUTPUT,
@StsSenderPaymentOK bit OUTPUT,
@StsUserKBAOk bit OUTPUT,
@WireId int OUTPUT,
@OriAmount MONEY OUTPUT,
@DestAmount MONEY OUTPUT,
@SndFullName VARCHAR(150) OUTPUT, 
@OriCurrency char(3) OUTPUT,   
@AgSenderCountry VARCHAR(30) OUTPUT, 
@AgSenderState VARCHAR(40) OUTPUT, 
@AgSenderCity VARCHAR(40) OUTPUT, 
@RcvFullName VARCHAR(150) OUTPUT, 
@DestCurrency char(3) OUTPUT,   
@DestCountry VARCHAR(30) OUTPUT, 
@DestState VARCHAR(40) OUTPUT,
@DestCity VARCHAR(40) OUTPUT,  
@SndPhone VARCHAR(20) OUTPUT, 
@RcvPhone VARCHAR(40) OUTPUT,    
@DeptBankName VARCHAR(60) OUTPUT,
@AccountNumber VARCHAR(20) OUTPUT,  
@AgPayerCode VARCHAR(10) OUTPUT
...
BEGIN TRY  
  SELECT @WireId                  = WireId,
   ...
   IF @@ROWCOUNT = 0
      BEGIN
		...
		ELSE SET @LogErrorMessage = 'No se pudo encontrar el wire para el pin '+@PinNumber
		RETURN
	  END
  ...
  --FASE 3
   ...
   IF @@ROWCOUNT = 0
      BEGIN
		...
		ELSE SET @LogErrorMessage = 'No se pudo encontrar el webagent user '+convert(varchar,@WebAgentUserId)
		RETURN
	  END
  ...
  IF @StsComplianceOk = 0
   ...
   IF @StsCancel = 1
	   BEGIN
	     SET @WireAvailableDate = NULL
		 SET @MsgToShow = 'CancelledWireInFinalUpdate'
	   END
	ELSE 
		BEGIN  --OJO EN PRODUCCION VOLVER A PONERLO
...
BEGIN CATCH
    ...
    SELECT 
        @ErrorMessage = 'Bridge_GetFinalUpdateData - '+ERROR_MESSAGE(),
