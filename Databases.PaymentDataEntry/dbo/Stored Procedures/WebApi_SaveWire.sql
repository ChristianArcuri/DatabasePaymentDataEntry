CREATE PROCEDURE [dbo].[WebApi_SaveWire]  -- Comes from WebAgent_WireCreate_Fase3_WhiteLabel_RcvIdType_Gateway
...
@DeliveryType CHAR(1),
@OriCountryAbbr CHAR(3),
@OriStateAbbr CHAR(3),   
@ReceiverBankAccountId INT,
@ReceiverBankName VARCHAR(30),
@UserPaymentMethodId INT,
@DeptBankAdditionalInfo VARCHAR(60), 

@SenderPaymentMethodDesc VARCHAR(50),
 ...
 -- INSERT INTO DebugGeru VALUES  (@WebAgentTransationId)  
  ...
  IF @@ROWCOUNT <> 0  
     BEGIN  
       ...
       SET @UserErrorMessage =  dbo.fnc_EcoMessage_withLanguage ('10860', @CurrentWebLanguageId)   
...
SELECT @ChannelId             = ChannelId   
	  ...
	  ,@LegalEntityCode        = wire.LegalEntityCode
	  ,@DeliveryType           = DeliveryType
	  ,@OriCountryAbbr         = OriCountryAbbr
	  ,@OriStateAbbr           = OriStateAbbr
	  ,@ReceiverBankAccountId  = wire.ReceiverBankAccountId
	  ,@ReceiverBankName       = C.BankName
	  ,@UserPaymentMethodId    = wire.UserPaymentMethodId
	  ,@DeptBankAdditionalInfo = DeptBankAdditionalInfo

	  ,@SenderPaymentMethodDesc = UPMI.SenderPaymentMethodDesc
	  ,@SenderPaymentMethodId = UPMI.SenderPaymentMethodId
      ,@CardTypeCode = UPMI.CardTypeCode
      ,@LastFourOfCard = UPMI.LastFourOfCard
	  ,@AgencyId = A.AgencyId
	  ...
	  @BrCity = B.BrCity,
      @BrState = B.BrState,
      @BrCountry = B.BrCountry

	FROM PaymentDataEntry.dbo.WebAgent_WireInPreparation AS wire
	LEFT JOIN SqlMain.WireTransac.dbo.WebAgent_UserPaymentMethodInfo UPMI  ON wire.[UserPaymentMethodId] = UPMI.[UserPaymentMethodInfoId]
	LEFT JOIN [Wiresearch].[dbo].[Agencies] A ON wire.AgPayerId = A.AgencyId
	LEFT JOIN [Wiresearch].[dbo].[Branches] B ON wire.BranchId = B.BranchID
	LEFT JOIN SqlMain.WireTransac.dbo.ReceiverBankAccounts C ON wire.ReceiverBankAccountID = C.ReceiverBankAccountID
 ...
 IF @@ROWCOUNT = 0  
   ...
   SET @WireResult  = 500  
      SET @ErrorCode = '11305'  
      SET @UserErrorMessage =  dbo.fnc_EcoMessage_withLanguage ('11305', @CurrentWebLanguageId)   
	  SET @LogErrorMessage = dbo.fnc_EcoMessage_withLanguage ('11305', @CurrentWebLanguageId) 
   RETURN  
 END  
  ...
  DECLARE @RcCalcDestAmount money  
   ...
   if Abs(ROUND(@ReCalcTotalAmount, 2) - @WireTotalAmount) > 0.01  
    begin  
      SET @WireResult  = 500  
      SET @ErrorCode = '10838'  
      SET @UserErrorMessage =  dbo.fnc_EcoMessage_withLanguage ('10838', @CurrentWebLanguageId)    
      SET @LogErrorMessage = dbo.fnc_EcoMessage_withLanguage ('10838', @CurrentWebLanguageId)  
   RETURN  
    end  
   ...
   begin  
      SET @WireResult  = 500  
      SET @ErrorCode = '10837'  
      SET @UserErrorMessage =  dbo.fnc_EcoMessage_withLanguage ('10837', @CurrentWebLanguageId) 
      SET @LogErrorMessage = dbo.fnc_EcoMessage_withLanguage ('10837', @CurrentWebLanguageId)   
   RETURN  
    end  
...
BEGIN TRY    
 ...
 if @@ROWCOUNT = 0  
  ...
  SET @WireResult  = 500  
  SET @ErrorCode = '10917'  
  SET @UserErrorMessage =  dbo.fnc_EcoMessage_withLanguage ('10917', @CurrentWebLanguageId)  
  SET @LogErrorMessage = dbo.fnc_EcoMessage_withLanguage ('10917', @CurrentWebLanguageId) 
  RETURN  
  END  
 ...
 IF @PointsTranType = 'WIRE'  
  ...
  INSERT INTO Wires with(rowlock)  
           (
				LSenderId ,
				LReceiverId ,
				--> WebAgent_WireInPreparation
				AgSenderId ,
				AgSenderCode,
				-- <
				AgSenderSeq , -- NO SE VER
				--> WireTransac.dbo.Senders
				AgSenderState ,
				AgSenderCity ,
				AgSenderCountry,  
				-- <
				...
				AgPayerCode ,
				DestCountry ,
				DestState, -- Si es vacio o null, setea del branch
				DestCity , -- Si DestState es vacio o null, setea del branch
				BranchId ,
				SenderId ,
				OnBehalfId,  
				ReceiverId ,
				...
				WireReadyToChargeSender,
				WebAgentUserId,  
				UserPaymentMethodInfoId,
				WireIPAddress,
				WireFromState,
				IPDetectedState,
				WebAgentTransationId,
				StsSenderPaymentOk,  
				CollectionType, 
				StsFraudCheckOk,
				CollectionStatus,
				InstantAchOK,StsCreditOk,
				DeviceFingerprint,
				ChannelId,
				PartnerId,
				...
				SndEmployerPhone,
				StyleId
			) --campos nuevos del Web Agent Wire  
     VALUES  
  ...
  IF(@WirePurposeId IS NOT NULL)
	BEGIN
	  INSERT INTO WiresPurposes
	  (
		WireId,
		WirePurposeId
	  )
  ...
  BEGIN TRY
	  DELETE FROM [Wiresearch].[dbo].[WebApi_SenderReceiverWireShortcut]
	  WHERE WebAgentUserId = @WebAgentUserId and SenderId = @SenderId and ReceiverId = @ReceiverId

	  INSERT INTO [Wiresearch].[dbo].[WebApi_SenderReceiverWireShortcut] with(rowlock)  (
		   [Id]
		  ,[WebAgentUserId]
		  ,[SenderId]
		  ,[ReceiverId]
		  ...
		  ,[BranchId]
		  ,[UserPaymentMethodId]
		  ,[DeptBankAdditionalInfo]
		  ,[wireId]
		  ,[SenderPaymentMethodDesc] 
		  ,[CardTypeCode]
		  ,[LastFourOfCard]
		  ,[PayerId]
		  ,[PayerCode]
...
-- Relationship between PreparationId and WebAgentTransationId... Luis Gamez
