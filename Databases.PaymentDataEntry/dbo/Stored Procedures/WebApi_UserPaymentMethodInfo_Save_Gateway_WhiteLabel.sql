CREATE PROCEDURE [dbo].[WebApi_UserPaymentMethodInfo_Save_Gateway_WhiteLabel]
@UserPaymentMethodInfoId int OUTPUT,
@WebAgentUserId int,
@PreparationId uniqueidentifier,  -- NEW WHITE LABEL
  ...
  IF @SenderPaymentMethodId = 2 --ACH
     BEGIN
	   ...
	   OR (rtrim(@AccountType) NOT IN( 'S','C'))
		...
		-- Verifies or validates account number - Luis Gamez
		-- It also checks if ACH payment method is duplicated
		-- if it is, then it returns 0 - Luis Gamez
        DECLARE @ValidResultOK int
        EXEC WireSearch.dbo.GIACT_ValidateAccountNumber_WhiteLabel 
														@WebAgentUserId ,
													    @PreparationId, 
														@CurrentWebLanguageId ,
		                                                @AccountNumber,
														@AbaCode,
														@ValidResultOK OUTPUT,
														@ErrorCode OUTPUT,
		...
		-- Once verified, it checks if payment info with verified account number is of status D - Luis Gamez
		...
		SELECT @UsersSameAccount = COUNT(DISTINCT i.webagentuserid)
		  FROM SqlMain.WireTransac.dbo.WebAgent_UserPaymentMethodInfo as i
		       INNER JOIN WireSecurity.dbo.WebAgent_Users as u on i.WebAgentUserId = u.WebAgentUserId and u.PartnerId = @PartnerId
		 Where i.SenderPaymentMethodId     = 2
		   and i.AbaCode                   = @AbaCode
		   and i.AccountNumber             = @AccountNumber
		   and i.WebAgentUserId           <> @WebAgentUserId
		   and i.SenderPaymentMethodStatus = 'A'
     ...
     BEGIN
	   ...
	   OR (@GatewayId = 0)
	      BEGIN
			...
			SET @UpdResult = 1
	        SET @UserErrorMessage  = ''
	        SET @LogErrorMessage   = 'Please provide the gateway to be used'
			RETURN
		  END
	   IF (@GatewayToken IS NULL)
	   OR (rtrim(@GatewayToken) = '')
	      BEGIN
		    SET @ErrorCode = ''  
			SET @UpdResult = 1
	        SET @UserErrorMessage  = ''
	        SET @LogErrorMessage   = 'Please generate the token first'
			RETURN
		  END
...
IF (@SenderPaymentMethodId in (3,4)) AND (@UserPaymentMethodInfoId is null )
	Begin
		SET @UserPaymentMethodInfoId  = 0
	END
...
IF @UserPaymentMethodInfoId = 0
       BEGIN
	      EXEC SqlMain.WireTransac.dbo.WebAgent_UserPaymentMethodInfo_Create_Gateway @WebAgentUserId ,
		                                                                     @LegalEntityCode,
	   ...
	   END
  ELSE  BEGIN
			 EXEC SqlMain.WireTransac.dbo.WebAgent_UserPaymentMethodInfo_Update_Gateway @UserPaymentMethodInfoId ,
															@WebAgentUserId ,
															@SenderPaymentMethodId ,
															@AbaCode ,
															@AccountNumber ,
            ...
            END
	--Select @UserPaymentMethodInfoId as PaymentMehtodId,
	--	   @UpdResult as UpdateResult
END