CREATE PROCEDURE [dbo].[WebAgent_LinkUserAndPreparationId]
@WebAgentUserId int,
...
BEGIN
  ...
  DECLARE @SndCreateResult int
  DECLARE @ErrorCode varchar(10)
  DECLARE @UserErrorMessage varchar(300)
  ...
  IF @UserStatus <> 'A'
	 ...
	 IF @SenderId = 0
     BEGIN
		EXEC WebAgentSenderCreateFromConsumerApp @WebAgentUserId, @SenderId OUTPUT, @SndCreateResult OUTPUT,
																				    @ErrorCode		 OUTPUT,
																				    @UserErrorMessage OUTPUT,
																				    @LogErrorMessage  OUTPUT
		IF @SndCreateResult = 500
		 RETURN
	 END
