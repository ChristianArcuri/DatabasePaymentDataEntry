CREATE PROCEDURE [dbo].[WebAgent_GetConfirmationpageMessageToShow_withKBA_Credit_fase3_3_TranTypeID_KBAFix]
@ComplianceOk bit,
@StsFraudCheckOk bit,
@StsUserKBAOk bit,
@StsCreditOK int, --New
...
AS
BEGIN
  IF (@StsUserKBAOk = 0 OR @StsCreditOK = 0) --Retenido x Lexis Nexis o Limite de Credito Llamar a Departamento de FLP
     BEGIN
	   IF @CurrentWebLanguageId = 1
	        SET @MsgToShow = 'Your Transaction was processed successfully. However, we need additional information before completing your wire transfer. Please contact us by calling 1-866-999-3175' --Obtenerlo configurable
  ...
  IF @ComplianceOk = 0 -- Retenido x Compliance 
	...
	BEGIN
	   IF @CurrentWebLanguageId = 1
	             SET @MsgToShow = 'Transaction successful. We''re processing your information and will notify you when the funds are available for pick-up. For questions contact 1-866-999-3175' --Obtenerlo configurable
	        ELSE SET @MsgToShow = 'Transacción exitosa, Estamos procesando tu informacion y te notificaremos en cuanto esté disponible para el cobro. Para preguntas, por favor contáctanos al 1-1-866-999-3175' --Obtenerlo configurable
            RETURN
	 END
