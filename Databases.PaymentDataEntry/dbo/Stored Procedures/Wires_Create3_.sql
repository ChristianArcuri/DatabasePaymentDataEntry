...
CREATE procedure [dbo].[Wires_Create3_]
(
    --Senders
	@SenderId int,
	@SenderGroupId int,
...
BEGIN TRY  
  ...
  if @InsSender = 1
    begin
	  INSERT INTO Senders with(rowlock) 
				  ...
				  SndIdTypeName, SndIdNumber, SndIdCountry, SndIdState, SndIdExpirationDate, SndEmail)
		   VALUES
			     (@SenderId, @SenderGroupId, @SndFullName, @SndFirstName, @SndLast1, @SndLast2, 
				  @SndAddress, @SndCountry, @SndState, @SndCity, @SndZip,
				  @SndPhone, @IsCellPhone, @SndLastVersionId, @SndNoSecLastName, 
				  @SndIdTypeName, @SndIdNumber, @SndIdCountryName, @SndIdStateName, @SndIdExpirationDate, @SndEmail )
 ...
 BEGIN TRY   
  ...
  -- exec spi_Error_LOG 'CREATE WIRE'
   ...
   INSERT INTO dbo.ErrorLog (ProcessName, ErrorMsg) values('Wires_Create3', @ERROR_MSG)
