CREATE procedure [dbo].[Wires_CreateAWireTransferToken]
(
    --Senders
	@SenderId int,
	@SenderGroupId int,
...
as  
  ...
  insert into Test([Msg], [NUM]) values('Wires_CreateAWireTransferToken' + @AgSenderCode , 1)
 ...
 BEGIN TRY   
  ...
  INSERT INTO Wires with(rowlock)
           (LSenderId ,LReceiverId ,AgSenderId ,AgSenderCode, 
            ...
            LoyaltyCardNumber,WirePoints,WirePointsSign, AcumLoyaltyPoints)
     VALUES
           (@LSenderID, @LReceiverID, @AgSenderId, @AgSenderCode,
            @AgSenderSeq, @AgSenderState, @AgSenderCity, @AgSenderCountry,
            @AgPayerId, @AgPayerCode, @DestCountry, @DestState,
...
*/
 ...
 --  exec spi_Error_LOG 'CREATE WIRE CD'
    if @@TRANCOUNT > 0
      rollback;
	INSERT INTO dbo.ErrorLog (ProcessName, ErrorMsg) values('res_CreateAWireTransferToken', @ERROR_MSG)
  END CATCH  
END TRY
BEGIN CATCH
   exec spi_Error_LOG 'CREATE WIRE CD'
