...
CREATE procedure [dbo].[Wires_CreateWCF]
(
    --Senders
	@SenderId int,
	@SenderGroupId int,
...
as  
  ...
  EXEC VerifyToken2 @ToKen, @AgSenderCode, @AgPayerCode, 1, @Result output, @Delay output 
  ...
  INSERT INTO Wires with(rowlock)
     ...
     VALUES
           (@LSenderID, @LReceiverID, @AgSenderId, @AgSenderCode,
            ...
            @NewTelewire,@ComputerIPAddress)
            
  select @WireId = SCOPE_IDENTITY()

   if (suser_sname() <> 'globalcashuser')
   or (@Result <> 0 ) 
    begin
        INSERT INTO dbo.WirePossibleFraud with(rowlock) (WireId, TokenResult, S, Token) values(@WireId, @Result, 9, @Token) 
