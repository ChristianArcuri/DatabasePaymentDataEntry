...
CREATE   procedure [dbo].[Wires_Create_RcvIdType_1025WithoutSign_FXRedesign]
(
    --Senders
	@SenderId int,
	@SenderGroupId int,
	...
	-- pricing redesign new Prc_WirePlans columns
	@AgencyPricingId INT,
  ...
  if @InsSender = 1
    BEGIN
	  INSERT INTO Senders with(rowlock) 
				  ...
				  SndIdTypeName, SndIdNumber, SndIdCountry, SndIdState, SndIdExpirationDate, SndEmail,OptStatus, OptStatusDate)
		   VALUES
			     (@SenderId, @SenderGroupId, @SndFullName, @SndFirstName, @SndLast1, @SndLast2, 
				  @SndAddress, @SndCountry, @SndState, @SndCity, @SndZip,
				  @SndPhone, @IsCellPhone, @SndLastVersionId, @SndNoSecLastName, 
				  @SndIdTypeName, @SndIdNumber, @SndIdCountryName, @SndIdStateName, @SndIdExpirationDate, @SndEmail ,@OptStatus,getdate())
    ...
    BEGIN
 	 UPDATE Senders with(updlock)
	    SET SenderId = @SenderId,     SndFullName = @SndFullName, SndFirstName = @SndFirstName,
            ...
            OptStatus = @OptStatus,OptStatusDate= getdate()
 ...
 BEGIN TRY   
  ...
  INSERT INTO Wires with(rowlock)
           (LSenderId ,LReceiverId ,AgSenderId ,AgSenderCode, 
			...
			F1025AgentFullName, AgencyPricingId, AgencyPricingDetailId, FxBaseId, FXPromotionId, FxBase, AgencyFXPointsFromBase, 
			AgencyPromoFXPoints, FXBasePromotionId, FxBasePromotion
			)
     VALUES
           (@LSenderID, @LReceiverID, @AgSenderId, @AgSenderCode,
            @AgSenderSeq, @AgSenderState, @AgSenderCity, @AgSenderCountry,
			...
			@1025AgentFullName, @AgencyPricingId, @AgencyPricingDetailId, @FxBaseId, @FXPromotionId, @FxBase, @AgencyFXPointsFromBase, 
			@AgencyPromoFXPoints, @FXBasePromotionId, @FxBasePromotion
			)
