CREATE PROCEDURE [dbo].[WebApi_UserRegistration_WithDataValidResult_WhiteLabel_v2]
@WebAgentUserId int OUTPUT,
@ChannelId int,
@SenderId int OUTPUT,
...
@UserPassword varchar(100),	--New
@Salt varchar(100),			--New
@FacebookId varchar(50),	--New
...
BEGIN
  ...
  SET @SenderId = 0
  SET @SameSenderId = 0
  SET @WebAgentUserId = 0
BEGIN TRY  

	IF (@WebAgentUserId = 0)
	BEGIN
		SELECT @WebAgentUserId=(MAX(WebAgentUserId) + 1) FROM WireSecurity.dbo.WebAgent_Users
	END --Esto es porque la columna WebAgentUserId tabla de WebAgent_Users no es un Identity porque venia de Azure
--======================================================================================
  ...
  --SET @SndCellPhone = dbo.JustNum(@SndCellPhone)
