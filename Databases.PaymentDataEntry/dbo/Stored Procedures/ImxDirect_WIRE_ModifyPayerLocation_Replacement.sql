CREATE PROCEDURE [dbo].[ImxDirect_WIRE_ModifyPayerLocation_Replacement]
(	
	...
	--OUTPUT
	@ModifyPayerLocationResult	INT				OUTPUT, --- 0 = ok, 1 = no grabo error no controlado, 500 Error de alguna validacion
...
DECLARE 
		...
		--- Set initial values
		SET @ModifyPayerLocationResult   = 0
		SET @ErrorCode					 = ''
	...
	BEGIN TRY 
		...
		SELECT TOP 1
				...
				@Nationality		= ss.SndCitizenship,
				
				-- Receiver  -- Receiver,
				--@RcvPhone				= R.RcvPhone,
				@RcvFirstName			= R.RcvFirstName,
				@RcvLast1				= R.RcvLast1,
				@RcvLast2				= R.RcvLast2,
		...
		BEGIN
			...
			SET @ModifyPayerLocationResult       = 500
		...
		BEGIN
					EXEC WireSearch.[dbo].[ImxDirect_WIRE_GetCharges_V6]
										...
										0,
										@ModifyPayerLocationResult OUTPUT,
										@ErrorCode				   OUTPUT,
					...
					IF @ModifyPayerLocationResult > 0
		...
		EXEC PaymentDataEntry.[dbo].[ImxDirect_WIRE_CreateAWireTransfer_V4]
					...
					@ReplacementReasonID =  2, --> Giro de reemplazo por cambio de Locacion de pagador (10-05-2021)
	...
	BEGIN CATCH
		...
		SET @ModifyPayerLocationResult = 1
		...
		SET @LogErrorMessage  =  'ImxDirect_WIRE_ModifyPayerLocation_Replacement - '+ ERROR_MESSAGE()
	END CATCH