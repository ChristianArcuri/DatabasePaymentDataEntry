...
CREATE   procedure [dbo].[BridgeGetWiresToProc_ODDEVEN_FXRedesign] @EvenWireId bit
as

 set nocount on;
 --hola
 --insert into debug (str,value) values('esta','OK')
    ...
    SELECT --top 1 
          ...
          ,WirePoints, WirePointsSign, WiresAlreadyCountGUID, W.LoyaltyCardNumber, OptStatus, OptStatusDate ,S.OptStatusPromo
		  ...
		  w.StsUserKBAOk,w.StsCreditOK, ISNULL(DeviceFingerprint,'') as DeviceFingerprint, SndEmployerName, SndEmployerPhone, SenderIdRecId,
		  w.AgencyExtraFee, w.WireSenderPaymentMethodFee, w.CashBackAmount, w.TransacTotalAmount,
--          CR.CardType, CR.LastCardDigits, CR.NameOnCard, CR.PnRef, CR.AuthCode,	CR.ResultCode as TransactionErrorCode, CR.ErrorMsg,
		  cast((case W.CardDirectProvider when 1 then CR.CardType when 2 then CENPos.CardType else null end) as varchar(50)) as CardType, 
		  cast((case W.CardDirectProvider when 1 then CR.LastCardDigits when 2 then SubString(CENPos.CardNumber, 1, 20) else null end) as varchar(20)) as LastCardDigits,
		  cast((case W.CardDirectProvider when 1 then CR.NameOnCard when 2 then CENPos.NameOnCard else null end) as varchar(50)) as NameOnCard, 
		  cast((case W.CardDirectProvider when 1 then CR.PnRef when 2 then CENPos.ReferenceNumber else null end) as varchar(100)) as PnRef, 
		  cast((case W.CardDirectProvider when 1 then CR.AuthCode when 2 then CENPos.AuthCode else null end) as varchar(100)) as AuthCode, 
		  cast((case W.CardDirectProvider when 1 then CR.ResultCode when 2 then CENPos.ResponseResult else null end) as varchar(10)) as TransactionErrorCode, 
		  cast((case W.CardDirectProvider when 1 then CR.ErrorMsg when 2 then CENPos.ResponseMessage else null end) as varchar(max)) as ErrorMsg,
		  ISNULL(ChannelId,0) as ChannelId, ISNULL(PartnerId,0) as PartnerId,
		  ISNULL(W.AgPayerRcvIdTypeRecordId,0) as AgPayerRcvIdTypeRecordId, ISNULL(W.RcvIdNumber,'') as RcvIdNumber, W.CardDirectProvider,
		  ISNULL(w.F1025AgentFullName,'') as F1025AgentFullName,
          AgencyPricingId,AgencyPricingDetailId,FxBaseId,FXPromotionId,FxBase,AgencyFXPointsFromBase,AgencyPromoFXPoints,FXBasePromotionId,FxBasePromotion,
		  ISNULL(W.StyleId,0) as StyleId, R.RcvDOB, S.Citizenship
      FROM Wires as W with(nolock) 
           ...
           LEFT OUTER JOIN CardChargeResult CR on CR.WireId = W.WireId 
		                                      and CR.CardChargeId = (select max(T02.CardChargeId)
		                                                             from CardChargeResult T02 
																	 where T02.WireId = W.WireId)
           LEFT OUTER JOIN ViewCENPosCardResult CENPos on CENPos.WireId = W.WireId
		                                             and CENPos.ResultId = (select max(T03.ResultId) 
					                                                       from ViewCENPosCardResult T03 
											                               where T03.WireId = CENPos.WireId)																	   
		   LEFT OUTER JOIN WebAgent_WireFraudCheck as f on W.WireId = F.WireId
       WHERE Done = 0 
	    -- AND dbo.fn_IsEvenNumber(W.WireId) = @EvenWireId 
		 AND ((@EvenWireId  = 1 AND (W.WireId % 2) = 0 ) OR (@EvenWireId  = 0 AND (W.WireId % 2) <> 0 ))
